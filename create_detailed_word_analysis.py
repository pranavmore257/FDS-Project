...
# Add subtitle
doc.add_heading('Detailed Implementation of Dijkstra, Bellman-Ford, and Floyd-Warshall Algorithms', 0)
...
doc.add_paragraph('Visual representation of the Pune locations network:')
doc.add_paragraph('''
...
doc.add_paragraph('Finds shortest path from source to all vertices using a priority queue (min-heap). Always selects the unvisited vertex with smallest distance.')
...
code_para = doc.add_paragraph()
code_para.add_run('Final Result: Katraj → Wakad (22.1 km)').bold = True
...
doc.add_heading('Initial Next Matrix:', level=3)
code_para = doc.add_paragraph()
code_para.add_run('          Katraj Swargate Baner Wakad').bold = True
code_para = doc.add_paragraph()
code_para.add_run('Katraj      -    Swargate  Baner  Wakad')
code_para = doc.add_paragraph()
code_para.add_run('Swargate Katraj     -      Baner  Wakad')
code_para = doc.add_paragraph()
code_para.add_run('Baner    Katraj  Swargate    -     Wakad')
code_para = doc.add_paragraph()
code_para.add_run('Wakad    Katraj  Swargate  Baner    -')
...
hdr_cells[1].text = 'Shortest Path'
hdr_cells[2].text = 'Distance'
hdr_cells[3].text = 'Execution Time'
hdr_cells[4].text = 'Key Steps'
...
row_cells[4].text = '4 vertex extractions'
...
row_cells[4].text = '3 iterations × 12 edges'
...
row_cells[4].text = '4³ = 64 matrix operations'

doc.add_heading('Why All Algorithms Found the Same Result', level=2)
doc.add_paragraph('1. Direct Edge: Katraj → Wakad (22.1 km) is already the shortest')
doc.add_paragraph('2. Alternative Path: Katraj → Baner → Wakad = 15.3 + 6.8 = 22.1 km (equal distance)')
doc.add_paragraph('3. Longer Path: Katraj → Swargate → Baner → Wakad = 8.2 + 12.5 + 6.8 = 27.5 km')

doc.add_heading('Algorithm Behavior Analysis', level=2)

doc.add_heading('Dijkstra\'s Algorithm', level=3)
doc.add_paragraph('• Greedy Approach: Always picks the closest unvisited vertex')
doc.add_paragraph('• Efficient: Uses priority queue for O((V + E) log V) complexity')
doc.add_paragraph('• Limitation: Cannot handle negative weights (not needed here)')
doc.add_paragraph('• Path Quality: Found optimal path by exploring vertices in order of distance')

doc.add_heading('Bellman-Ford Algorithm', level=3)
doc.add_paragraph('• Exhaustive: Relaxes all edges |V|-1 times')
doc.add_paragraph('• Robust: Can handle negative weights and detect cycles')
doc.add_paragraph('• Slower: O(V × E) complexity, but performed well here due to small graph')
doc.add_paragraph('• Path Quality: Guaranteed optimal after sufficient iterations')

doc.add_heading('Floyd-Warshall Algorithm', level=3)
doc.add_paragraph('• Comprehensive: Computes all-pairs shortest paths')
doc.add_paragraph('• Dynamic Programming: Uses intermediate vertices to improve paths')
doc.add_paragraph('• Memory Intensive: O(V²) space complexity')
doc.add_paragraph('• Path Quality: Found optimal path through systematic matrix updates')

doc.add_heading('Key Insights from This Example', level=2)
doc.add_paragraph('1. Direct vs Indirect Paths: Sometimes the direct path is already optimal')
doc.add_paragraph('2. Algorithm Trade-offs:')
doc.add_paragraph('   – Dijkstra: Fastest for single-source in sparse graphs')
doc.add_paragraph('   – Bellman-Ford: Most versatile (handles negatives)')
doc.add_paragraph('   – Floyd-Warshall: Best for multiple queries (all-pairs)')
doc.add_paragraph('3. Real-world Application: In Pune\'s road network, the direct Katraj-Wakad route (22.1 km) is optimal compared to going through other areas')
doc.add_paragraph('4. Performance Variations: Despite finding the same result, execution times vary significantly based on algorithmic approach')

doc.add_heading('Conclusion', level=1)
doc.add_paragraph('All three algorithms correctly identified the shortest path from Katraj to Wakad as 22.1 km, demonstrating their effectiveness for real-world route optimization in Pune\'s urban landscape. The step-by-step analysis shows how each algorithm\'s unique approach leads to the same optimal solution while highlighting their computational differences and use cases.')

doc.add_paragraph('This detailed analysis provides a comprehensive understanding of how fundamental DSA algorithms work in practice, using real Pune locations to demonstrate their behavior, performance characteristics, and practical applications in route finding systems.')

# Save the document
doc.save('c:/Users/ACER/Documents/impleFDS/Algorithm_Step_by_Step_Analysis_DETAILED.docx')
print('Detailed Word document with step-by-step algorithm analysis created successfully!')
