...
# Add subtitle
doc.add_heading('Corrected Path Analysis: Katraj → Swargate → Wakad (ONLY via Swargate)', 0)
...
doc.add_paragraph('• Required Path: MUST go through Swargate ONLY')
...
doc.add_paragraph('''
    Katraj (8.2) Swargate (18.7) Wakad
     ...
     15.3  22.1   12.5       6.8
      |     \\       |           |
      Baner  Wakad  Katraj  Baner  Wakad
''')

doc.add_paragraph('REQUIRED PATH: Katraj → Swargate → Wakad')
doc.add_paragraph('Total distance: 8.2 + 18.7 = 26.9 km')
...
# Required vs Actual Analysis
doc.add_page_break()
doc.add_heading('IMPORTANT: Required vs Actual Path Analysis', level=1)

doc.add_heading('Required Path (ONLY via Swargate)', level=2)
doc.add_paragraph('• Katraj → Swargate → Wakad')
doc.add_paragraph('• Distance: 8.2 + 18.7 = 26.9 km')
doc.add_paragraph('• This path goes ONLY THROUGH SWARGATE as requested')

doc.add_heading('Actual Algorithm Results', level=2)
doc.add_paragraph('All three algorithms found: Katraj → Wakad (22.1 km)')
doc.add_paragraph('• This is DIRECT path, not through Swargate')
...
doc.add_paragraph('2. Direct path (22.1 km) < Path via Swargate (26.9 km)')
...
doc.add_heading('To Force Path Through Swargate Only', level=2)
doc.add_paragraph('If you specifically need the path to go through Swargate ONLY, you could:')
doc.add_paragraph('1. Remove the direct Katraj-Wakad edge from the graph')
doc.add_paragraph('2. Set the Katraj-Wakad edge weight to a very large number (e.g., 9999)')
doc.add_paragraph('3. Add a constraint that the path must include Swargate')
doc.add_paragraph('4. Use two-stage path finding: Katraj → Swargate + Swargate → Wakad')

doc.add_heading('Modified Example (Forcing Swargate-Only Path)', level=2)
...
doc.add_paragraph('')
doc.add_paragraph('To force ONLY via Swargate, also remove Katraj-Baner edge:')
doc.add_paragraph('• Then only available path: Katraj → Swargate → Wakad = 26.9 km')
...
row_cells[0].text = 'Required (via Swargate only)'
row_cells[1].text = 'Katraj → Swargate → Wakad'
row_cells[2].text = '26.9 km'
row_cells[3].text = 'N/A'
row_cells[4].text = 'Via Swargate Only'

doc.add_heading('Key Takeaways', level=2)
doc.add_paragraph('1. All algorithms work correctly and found the mathematically shortest path')
doc.add_paragraph('2. Direct path (22.1 km) is shorter than path via Swargate (26.9 km)')
doc.add_paragraph('3. Algorithms are designed to minimize distance, not follow specific routes')
doc.add_paragraph('4. To force a specific path through Swargate only, modify the graph structure')
doc.add_paragraph('5. The step-by-step analysis shows how each algorithm processes vertices and edges')

doc.add_heading('Graph Modification for Required Path', level=2)
doc.add_paragraph('To get the required path Katraj → Swargate → Wakad:')
doc.add_paragraph('1. Remove edge: Katraj ↔ Wakad (22.1 km)')
doc.add_paragraph('2. Remove edge: Katraj ↔ Baner (15.3 km)')
doc.add_paragraph('3. Keep edge: Katraj ↔ Swargate (8.2 km)')
doc.add_paragraph('4. Keep edge: Swargate ↔ Wakad (18.7 km)')
doc.add_paragraph('5. Result: Only path available is Katraj → Swargate → Wakad (26.9 km)')

doc.add_heading('Conclusion', level=1)
doc.add_paragraph('This detailed analysis demonstrates how all three shortest path algorithms work on your Pune locations example. The algorithms correctly identified the direct Katraj-Wakad path (22.1 km) as the shortest route, which is mathematically optimal. However, since you specifically require the path to go through Swargate only, you would need to modify the graph structure by removing the direct Katraj-Wakad edge and the Katraj-Baner edge. This would force the algorithms to find the path Katraj → Swargate → Wakad (26.9 km). The step-by-step breakdown shows exactly how each algorithm processes vertices and edges to arrive at their optimal solution.')

doc.add_paragraph('The key insight is that shortest path algorithms are designed to find the mathematically shortest route, not to follow specific waypoints. To achieve your requirement of going through Swargate only, the graph structure must be modified to eliminate shorter alternatives.')

# Save the document
doc.save('c:/Users/ACER/Documents/impleFDS/Algorithm_Analysis_SWARGATE_ONLY.docx')
print('Final Word document with Swargate-only path analysis created successfully!')
